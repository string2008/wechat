...
class WXBizMsgCrypt
{
	...
	{
	    //var_dump($token); var_dump($encodingAesKey); var_dump($Corpid);
		$this->m_sToken = $token;
	...
	*/
	...
	{
		//logTrace("WXBizMsgCrypt::EncryptMsg---begin");
		//logTrace("WXBizMsgCrypt::EncryptMsg---m_sEncodingAesKey".$this->m_sEncodingAesKey);
		$pc = new Prpcrypt($this->m_sEncodingAesKey);
		//logTrace("WXBizMsgCrypt::EncryptMsg---m_sEncodingAesKey");
		...
		//logTrace("WXBizMsgCrypt::EncryptMsg---2");
		//生成安全签名
		$sha1 = new SHA1;
		...
		$signature = $array[1];
		//logTrace("WXBizMsgCrypt::EncryptMsg---3");
		//生成发送的xml
		$xmlparse = new XMLParse;
		$sEncryptMsg = $xmlparse->generate($encrypt, $signature, $sTimeStamp, $sNonce);
		//logTrace("WXBizMsgCrypt::EncryptMsg---sEncryptMsg:$sEncryptMsg");
		return ErrorCode::$OK;
	}
